...
''')
    ...
    if args.postcleanup:
        throughput.cleanup()
    if args.rotation:
        throughput.generate_report(list(set(iterations_before_test_stopped_by_user)), incremental_capacity_list, data=all_dataframes, data1=to_run_cxs_len, report_path=throughput.result_dir,coordinate_list=coord_list,angle_list=angle_list)
    else:   
