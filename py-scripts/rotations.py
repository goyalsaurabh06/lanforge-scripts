...
class Throughput(Realm):
    def __init__(self,
                 ...
                 total_resources_list=None, working_resources_list=None, hostname_list=None, username_list=None, eid_list=None,total_floors=0,
                 devices_available=None, input_devices_list=None, mac_id1_list=None, mac_id_list=None, overall_avg_rssi=None,rotation_enabled=None,robo_ip=None):
    ...
    def monitor(self, iteration, individual_df, device_names, incremental_capacity_list, overall_start_time, overall_end_time,current_coordinate,rotation_list=None,target_x=None, target_y= None,angle_list= None):
            ...
            if self.rotation_enabled:
                ...
                while True:
                    ...
                    if abs(angle - theta) <= 0.15:
                        logger.info("Rotated to angle  {}".format(final_angle))
                        break
                ...
                if self.dowebgui:
                    ...
                    if (current_time - previous_time).total_seconds() >= time_break:
                        ...
                        if self.group_name is None:
                            individual_df_for_webui.to_csv('{}/{}_throughput_data.csv'.format(runtime_dir,current_coordinate), index=False)
                        else:
        ...
        if iteration + 1 == len(incremental_capacity_list):
            if self.rotation_enabled:
                individual_df_data.extend([round(sum(download_throughput), 2), round(sum(upload_throughput), 2), sum(drop_a_per), sum(drop_a_per), iteration + 1, timestamp,
                                        overall_start_time.strftime("%d/%m %I:%M:%S %p"), timestamp, 0, ', '.join(str(n) for n in incremental_capacity_list), final_angle,'Stopped'])
            else:
                individual_df_data.extend([round(sum(download_throughput), 2), round(sum(upload_throughput), 2), sum(drop_a_per), sum(drop_a_per), iteration + 1, timestamp,
        ...
        if self.do_interopability is False:
            ...
            # Loop through iterations and build graphs, tables for each iteration
                    ...
                    for i in range(len(iterations_before_test_stopped_by_user)):
                        ...
                        #     .append(int(sig)*(-1))
                        # rssi_signal_data.append(signal_data)signal_data
                    ...
                    # Fetch download_data and upload_data based on load_type and direction
                        ...
                        if len(incremental_capacity_list) > 1:
                            ...
                            report.build_custom()
                        # print("uploadlist",download_values_list)
                        # print("uploadlist",upload_values_list)
                        # print("datasetingraph",data_set_in_graph)
                        report.set_obj_html(
                        ...
                        if not self.rotation_enabled:
                            ...
                            graph = lf_bar_graph_horizontal(_data_set=devices_data_to_create_bar_graph,
                                                            ...
                                                            _yaxis_name="Devices",
                                                            _graph_image_name=f"image_name{coordinate}",
                                                            _label=label_data,
                            ...
                            graph = lf_bar_graph_horizontal(_data_set=[rssi_data],
                                                            ...
                                                            _yaxis_name="Devices",
                                                            _graph_image_name=f"signal_image_name{coordinate}",
                                                            _label=['RSSI'],
                                                            _yaxis_categories=devices_on_running_trimmed,
                                                            _legend_loc="best",
                                                            _legend_box=(1.0, 1.0),
                        ...
                        if self.group_name:
                            ...
                            # print("bkdataframe",bk_dataframe)
...
''')
    ...
    optional.add_argument('--manual', help="If true the user gives the coordinate points", action='store_true')
    ...
    for index in range(len(loads_data)):
        ...
        #     coord_list=list(coordinate_dict.keys())
            # coord_list.sort()
        # To send a post request to the robot_server to start
        # url = 'http://'+robo_ip+':'+robo_port+'/inputs'     #used api inputs
        ...
        if args.manual:
            while True:
                if found:
                    user_input = input("Enter coordinate to test (or type 'exit' to quit): ").strip()
                    found=False
                    if user_input.lower() == 'exit':
                        break
                robot_status='Started'
                while(robot_status != 'Stopped'):
                    time.sleep(2)
                    url = 'http://'+ robo_ip +':'+robo_port+'/logs' # api used to get the bot status 
                    response= requests.get(url)
                    try:            
                        robot_status=response.json()['bot_status']
                    except:
                        logger.info("error while quiring robotLogs")
                        robot_status='Error'
                    if (robot_status =='Stopped'):              
                        url = 'http://'+ robo_ip +':'+robo_port+'/readNfc' # api used read the current coordinate
                        response = requests.get(url)
                        time.sleep(5)                
                        current_coordinate = str(response.json()['NFC_tag']['Coordinate'])
                        #try:
                        ...
                        with open(nav_data, 'r') as x:
                            navdata = json.load(x)
                        navdata['Canbee_location']=current_coordinate
                        with open(nav_data, 'w') as x:
                            json.dump(navdata, x, indent=4)
                        if current_coordinate == user_input:
                            # nav_data[args.test_name]=current_coordinate
                            found=True                     
                            tem_list=[]           
                            coord_list.append(current_coordinate)
                            individual_dataframe_column = []
                            ...
                            for i in range(len(to_run_cxs)):
                                ...
                                all_dataframes, test_stopped_by_user = throughput.monitor(i, individual_df, device_names, incremental_capacity_list, overall_start_time, overall_end_time,current_coordinate)
            ...
            abort=False
            position_url = 'http://'+robo_ip+'/reeman/position'
            ...
            for coord in coord_list:
                if test_stopped_by_user:
                    break
                passed_coord_list.append(coord)
                # print("coord")
                moverobo_url = 'http://'+robo_ip+'/cmd/nav_name'
                ...
                retries=0
                status_url= 'http://'+robo_ip+'/reeman/nav_status'
                while True:
                    matched = False
                    try:
                    ...
                    if goal == target_name and state == 3:
                        matched = True
                    elif "," in goal:
                        ...
                        if gx == target_x and gy == target_y and state == 3:
                            matched = True
                    if matched :
                        logger.info("Reached point {}".format(coord))
                        break
                ...
                for i in range(len(to_run_cxs)):
                    ...
                    # Monitor throughput and capture all dataframes and test stop status
                    all_dataframes, test_stopped_by_user = throughput.monitor(i, individual_df, device_names, incremental_capacity_list, overall_start_time, overall_end_time,coord,list_of_rotation,target_x,target_y,angle_list)
    ...
    if args.postcleanup:
        throughput.cleanup()
    if args.rotation:
        throughput.generate_report(list(set(iterations_before_test_stopped_by_user)), incremental_capacity_list, data=all_dataframes, data1=to_run_cxs_len, report_path=throughput.result_dir,coordinate_list=passed_coord_list,angle_list=angle_list)
    else:   
        throughput.generate_report(list(set(iterations_before_test_stopped_by_user)), incremental_capacity_list, data=all_dataframes, data1=to_run_cxs_len, report_path=throughput.result_dir,coordinate_list=passed_coord_list)
