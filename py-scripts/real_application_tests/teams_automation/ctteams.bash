...
if [[ "$(uname)" == "Linux" ]]; then
    ...
    if [ "$TYPE" == "host" ]; then
        DISPLAY=$DISPLAY ./vrf_exec.bash $IFACE "python3 teams_host.py --ip $IP $YTENV" > teams_host.log 2>&1
    
    elif [ "$TYPE" == "client" ]; then

        DISPLAY=$DISPLAY ./vrf_exec.bash $IFACE "python3 teams_client.py --ip $IP $YTENV" > teams_client.log 2>&1
    else
    ...
    if [ "$TYPE" == "host" ]; then
        python3 teams_host.py --ip $IP > teams_host.log 2>&1
    elif [ "$TYPE" == "client" ]; then
        python3 teams_client.py --ip $IP > teams_client.log 2>&1
    else
        echo "Invalid TYPE specified. Please use 'host' or 'client'."
        exit 1
    fi
