...
class TeamsAutomation(Realm):
    ...
    def updating_webui_runningjson(self, obj):
        ...
        file_path = self.path + "/../../Running_instances/{}_{}_running.json".format(
            self.lanforge_ip, self.test_name
        )
    ...
    def update_webui_data(self):
        if len(self.real_sta_hostname) == 0:
            ...
            obj = {"status": "Stopped", "configuration_status": "configured"}
    ...
    def select_real_devices(self, real_sta_list=None):
        ...
        if real_sta_list is None:
            ...
            if not self.do_webui:
                ...
                real_sta_list = [
                    sta.strip() for sta in real_sta_list.split(",") if sta.strip()
                ]
        ...
        self.real_sta_hostname = [
            self.real_sta_data_dict[name]["hostname"]
            for name in self.real_sta_data_dict
        ]
        self.hostname_os_combination = [
            f"{hostname} ({os_type})"
    ...
    def start_flask_server(self):
        ...
        def login_completed():
            if request.method == "POST":
                ...
                return jsonify(
                    {
                        "message": f"Updated login_completed status to {bool(login_completed_status)}"
                    }
                )
        ...
        try:
            self.app.run(
                host="0.0.0.0", port=5005, debug=True, threaded=True, use_reloader=False
            )
        except Exception as e:
