...
if [[ "$(uname)" == "Linux" ]]; then
    ...
    if [ "$TYPE" == "host" ]; then
        DISPLAY=$DISPLAY ./vrf_exec.bash $IFACE "python3 gmeet_host.py --upstream_port_ip $IP $YTENV"

    
    elif [ "$TYPE" == "client" ]; then
        DISPLAY=$DISPLAY ./vrf_exec.bash $IFACE "python3 gmeet_client.py --ip $IP $YTENV"
    else
    ...
    if [ "$TYPE" == "host" ]; then
        python3 gmeet_host.py --upstream_port_ip $IP

    elif [ "$TYPE" == "client" ]; then
        python3 gmeet_client.py --upstream_port_ip $IP
    else
        echo "Invalid TYPE specified. Please use 'host' or 'client'."
        exit 1
    fi
