...
# Robo = importlib.import_module("py-scripts.lf_robo_base_class")
...
class Throughput(Realm):
    def __init__(self,
                 ...
                 devices_available=None, input_devices_list=None, mac_id1_list=None, mac_id_list=None, overall_avg_rssi=None,rotation_enabled=None,robo_ip=None,angle_list=None):
    ...
    def monitor(self, iteration, individual_df, device_names, incremental_capacity_list, overall_start_time, overall_end_time,current_coordinate=None,cxlist=None):
        ...
        for j in  range(len(rotation_list)):
            ...
            if pause_angle:
                reached = self.robot.move_to_coordinate(current_coordinate)
                if not reached:
                ...
                if (datetime.now() - monitor_charge_time).total_seconds() >= 300:
                    ...
                    if pause:
                        ...
                        if self.rotation_enabled:
                            ...
                            rotation_moni =self.robot.rotate_angle(angle)
                        ...
                        previous_time=datetime.now()
                    monitor_charge_time = datetime.now()
                # print("previoustimeee",previous_time)
                if self.dowebgui:
                    ...
                    if (current_time - previous_time).total_seconds() >= time_break:
                        ...
                        if self.group_name is None and self.robo_ip:
                            ...
                            individual_df_for_webui.to_csv('{}/{}_overall_throughput.csv'.format(runtime_dir,current_coordinate), index=False)
                        else:
...
''')
    ...
    for index in range(len(loads_data)):
        ...
        if args.coordinate:
            ...
            passed_coord_list = []
            abort=False
            throughput.robot.create_waypointlist()
        ...
        for coord in coord_list:
            ...
            for i in range(len(to_run_cxs)):
                ...
                if args.robot_ip:
                    all_dataframes, test_stopped_by_user = throughput.monitor(i, individual_df, device_names, incremental_capacity_list, overall_start_time, overall_end_time,coord)
                else:
