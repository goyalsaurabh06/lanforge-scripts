...
class Throughput(Realm):
    ...
    def monitor(self, iteration, individual_df, device_names, incremental_capacity_list, overall_start_time, overall_end_time,current_coordinate):
        individual_df_for_webui = individual_df.copy()
        ...
        while datetime.now() < end_time:
            ...
            try:
                response = list(self.json_get('/endp/{}/list?fields=rx rate (last),rx drop %25,name,run,name'.format(','.join(cx_list_endp)))['endpoint'])
                # exit(0)
            ...
            for cx in cx_list:
                throughput[index][i] = [0, 0, 0, 0, "Stopped"]
                for j in response:
                    key, value = next(iter(j.items()))
                    Endp_Name_A = cx + '-A'
                    Endp_Name_B = cx + '-B'
                    if value['name'] == Endp_Name_A:
                        throughput[index][i][0] = value['rx rate (last)']
                        throughput[index][i][2] = value['rx drop %']
                    elif value['name'] == Endp_Name_B:
                        throughput[index][i][1] = value['rx rate (last)']
                        throughput[index][i][3] = value['rx drop %']
                    if value['name'] == Endp_Name_A or value['name']==Endp_Name_B:
                        throughput[index][i][4] = 'Run' if value['run'] else 'Stopped'
                i+=1
            if self.dowebgui:
                time.sleep(1)
                individual_df_data = []
    ...
    def build_line_graph(self, data_set, xaxis_name, yaxis_name, xaxis_categories, label, graph_image_name):
        ...
        if data_size <= 60:
            tick_positions = range(data_size)
        else:
            # Ensure 60 points including the first and last
            interval = max(1, (data_size - 2) // 58)
            tick_positions = [0] + list(range(interval, data_size - 1, interval)) + [data_size - 1]
    ...
    def generate_report(self, iterations_before_test_stopped_by_user, incremental_capacity_list,data1=None, report_path='', result_dir_name='Throughput_Test_report',
                        selected_real_clients_names=None,coordinate_list=None):
        ...
        if self.do_interopability == False:
            ...
            for coord in coordinate_list:
                filename = f"{coord}_throughput_data.csv"
                if os.path.exists(filename):
                    shutil.copy(filename, report_path_date_time)
            ...
            # Loop through iterations and build graphs, tables for each iteration
                ...
                for i in range(len(iterations_before_test_stopped_by_user)):
                    ...
                    graph_png = self.build_line_graph(
                                                    data_set=trimmed_data_set_in_graph,
                                                    xaxis_name="Time",
                                                    yaxis_name="Throughput (Mbps)",
                                                    xaxis_categories=self.trim_data(
                                                        len(data['TIMESTAMP'][data['Iteration'] == i + 1].values.tolist()),
    ...
    def trim_data(self, array_size, to_updated_array):
        if array_size <= 1:
            return to_updated_array
        step = (array_size - 1) / (array_size - 1)
        selected_indices = [round(i * step) for i in range(array_size)]
        updated_array = [to_updated_array[index] for index in selected_indices]
...
''')
    ...
    required.add_argument('--robot_ip', default='localhost', help='hostname for where Robot server is running')
    required.add_argument('--robot_port', default=5000, help='port Robot HTTP service is running on')
    ...
    required.add_argument('--manual', help="If true the user gives the coordinate points", action='store_true')
    ...
    required.add_argument('--coordinate',help="The coordinate dictionary consists points and their respective x and y values")
    ...
    for index in range(len(loads_data)):
        ...
        # print(base_dir,"basedirectory",nav_data,"navjson dir")
        found=True
        if args.manual:
            while True:
                if found:
                ...
                while(robot_status != 'Stopped'):
                    time.sleep(2)
                    url = 'http://'+ robo_ip +':'+robo_port+'/logs'
                    ...
                    if (robot_status =='Stopped'):              
                        url = 'http://'+ robo_ip +':'+robo_port+'/readNfc'
                        response = requests.get(url)
                        time.sleep(5)                
            ...
            while points:
                ...
                while(robot_status != 'Stopped'):
                    ...
                    if (robot_status =='Stopped'):              
                        ...
                        with open(nav_data, 'w') as x:
                            json.dump(navdata, x, indent=4)
                        if current_coordinate in points:
                            # nav_data[args.test_name]=current_coordinate                     
                            tem_list=[]           
                            points.remove(str(response.json()['NFC_tag']['Coordinate']))
                            individual_dataframe_column = []
                            for i in range(len(clients_to_run)):
    ...
    if args.postcleanup:
        throughput.cleanup()
    throughput.generate_report(list(set(iterations_before_test_stopped_by_user)), incremental_capacity_list, data1=to_run_cxs_len, report_path=throughput.result_dir,coordinate_list=coord_list)
