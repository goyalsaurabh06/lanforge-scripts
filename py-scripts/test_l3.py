...
./test_l3.py
        ...
        # Example : Command Line Interface to run Multicast robo test with Rotations
             ./test_l3.py
             --lfmgr 192.168.207.78
             --test_duration 1m
             --polling_interval 1s
             ...
             --test_tag test_l3
             --use_existing_station_list
             --existing_station_list 1.12.wlan0
             --cleanup_cx
             --tos BE
             --test_name Sample_test
             --dowebgui True
             --local_lf_report_dir /home/lanforge/local/interop-webGUI/results/Sample_test
             --robot_test
             --coordinate 21,29
             --rotation "10"
             --robot_ip 192.168.200.179

        # Example : Command Line Interface to run Multicast robo test without Rotations
             ./test_l3.py
             --lfmgr 192.168.207.78
             --test_duration 1m
             --polling_interval 1s
        ...
        # Example : Command Line Interface to run Multicast Real
            ./test_l3.py  
        ...
        # Example : Command Line Interface to run Multicast Virtual
            ./test_l3.py 
            --lfmgr 192.168.204.75 
            --endp_type mc_udp 
            --tos BE 
        ...
        # Example : Command Line Interface to run Multicast Real + Virtual
            ./test_l3.py 
            --lfmgr 192.168.204.75 
            --endp_type mc_udp 
            --tos BE 
        ...
        # Example : Command Line Interface to run Multicast Real + Existing
            ./test_l3.py 
            --lfmgr 192.168.204.75 
            --endp_type mc_udp 
            --tos BE 
        ...
        # Example : Command Line Interface to run Multicast Virtual + Existing (Be Cautios that dont while entering the sta_start_offset it need not same as existing stations)
            ./test_l3.py 
            --lfmgr 192.168.204.75 
            --endp_type mc_udp 
            --tos BE 
...
class L3VariableTime(Realm):
    ...
    def __init__(self,
        ...
        if not self.dataplane:
            ...
            if (self.client_type=="both" and not self.use_existing_station_list_virtual) or (not self.use_existing_station_lists and not self.use_existing_station_list_virtual) or (self.client_type=='virtual' and self.use_existing_station_list_virtual):
                for (
                        _radio_,
                        ssid_,
                        ssid_password_,
    ...
    def build(self, rebuild=False):
        ...
        if self.dataplane:
            ...
            for station_profile in self.station_profiles:
                ...
                if not rebuild:
                    # we are checking whether we are not working with real and existing stations means for virtual scenario
                    if not self.use_existing_station_lists and not self.use_existing_station_list_virtual:
                        ...
                        index += 1
                        self.virtual_clients.extend(station_profile.station_names)
                    #we are verifying whether we are dealing with both real and virtual or both virtual and existing stations and also in such case 
                    # the first station profile is real or existing stations profile so we skip by setting the start to false and continue
                ...
                if endp_data[endp_data_key]['type'] == 'Mcast' and endp_data_key in self.multicast_profile.created_mc:
                    if endp_data[endp_data_key]['tos'] == 'BK':
                        ...
                        if endp_data[endp_data_key]['a/b'] == "B":
                            ...
                            self.bk_os_type_A[-1]="-" if self.bk_client_type_A[-1]=="Virtual" else self.bk_os_type_A[-1]
                        ...
                        if endp_data[endp_data_key]['a/b'] == "B":
                            ...
                            self.be_os_type_A[-1]="-" if self.be_client_type_A[-1]=="Virtual" else self.be_os_type_A[-1]
                        ...
                        if endp_data[endp_data_key]['a/b'] == "B":
                            ...
                            self.vi_os_type_A[-1]="-" if self.vi_client_type_A[-1]=="Virtual" else self.vi_os_type_A[-1]
                        ...
                        if endp_data[endp_data_key]['a/b'] == "B":
                            ...
                            self.vo_os_type_A[-1]="-" if self.vo_client_type_A[-1]=="Virtual" else self.vo_os_type_A[-1]
    ...
    def filter_mlt_data(self):
        """
            This function filters client_dict_A to include only CXs with "MLT", excluding other CXs (throughput, QoS). This way, we can exclude 'NA' entries from the dict.
        """
    ...
    def generate_report(self, config_devices=None, group_device_map=None, iot_summary=None):
        ...
        # for real also the title Radio Configuration not needed
        if not self.robo_test and not self.real:
        ...
        #not needed as we filter through the mcast in the evaluate_qos and also the unicast also there so in such case the real devices are not shown
        ...
        if self.robo_test:
            ...
            if not hasattr(self, "multicast_robot_results") or not self.multicast_robot_results:
                ...
                for _, result in self.multicast_robot_results.items():
