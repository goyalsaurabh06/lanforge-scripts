...
'''
    ...
    EXAMPLE-6:
    Command Line Interface to run ping test by setting device specific Pass/Fail values in the csv file without configuration
    python3 lf_interop_ping.py --mgr 192.168.244.97 --real --target 192.168.1.3 --ping_interval 1 --ping_duration 1  --ssid RDT_wpa2 --security wpa2
    --passwd OpenWifi --device_csv_name device.csv --server_ip 192.168.244.97 --use_default_config
...
class Ping(Realm):
    ...
    def generate_report(self, result_json=None, result_dir='Ping_Test_Report', report_path='', config_devices='', groupdevmap={}):
        ...
        # packet_count_data = {}
        os_type = []
        logging.info('Device dataa: {}'.format(self.result_json.items()))
                    ...
                    for device in res_list:
                        ...
                        for row in rows:
                            if row['DeviceList'] == device and row['PingPacketLoss %'].strip() != '':
                                ...
                                break
                        if not found:
                            logging.info(f"Pass fail for Device {device} not found in CSV. Using default value 20")
                            test_input_list.append(20)
                    self.percent_pac_loss = []
    ...
    def generate_dataframe(self, groupdevlist, device_names, device_mac, device_channels, device_ssid, device_modes,
                           packets_sent, packets_received, packets_dropped, percent_pac_loss, test_input_list, pass_fail_list):
        # grouplist = []
        dev_names = []
        dev_mac = []
        dev_channels = []
