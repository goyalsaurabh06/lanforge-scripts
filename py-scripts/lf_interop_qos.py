...
"""
    ...
    python3 lf_interop_qos.py  --ap_name Cisco --mgr 192.168.244.97 --test_duration 1m --upstream_port eth1 --upload 100000
    --mgr_port 8080 --traffic_type lf_udp --tos "VI,VO,BE,BK" --ssid DLI-LPC992 --passwd Password@123
    --security open --config --server_ip 192.168.213.218
    ...
    python3 lf_interop_qos.py  --ap_name Cisco --mgr 192.168.244.97 --test_duration 1m --upstream_port eth1 --upload 100000
    --mgr_port 8080 --traffic_type lf_udp --tos "VI,VO,BE,BK" --ssid DLI-LPC992 --passwd Password@123
    --security open --config --expected_passfail_value 0.3 --server_ip 192.168.213.218

    EXAMPLE-7:
    Command Line Interface to run upload scenario by setting device specific Pass/Fail values in the csv file without configuring the devices
    python3 lf_interop_qos.py  --ap_name Cisco --mgr 192.168.244.97 --test_duration 1m --upstream_port eth1 --upload 100000
    --mgr_port 8080 --traffic_type lf_udp --tos "VI,VO,BE,BK" --ssid DLI-LPC992 --passwd Password@123
    --security open --device_csv_name device_config.csv --server_ip 192.168.213.218
    ...
    python3 lf_interop_qos.py  --ap_name Cisco --mgr 192.168.244.97 --test_duration 1m --upstream_port eth1 --upload 100000
    --mgr_port 8080 --traffic_type lf_udp --tos "VI,VO,BE,BK" --file_name g219 --group_name grp1 --profile_name Open3
    --expected_passfail_value 3 --wait_time 30
    ...
    def generate_individual_graph(self, res, report, connections_download_avg, connections_upload_avg, avg_drop_a, avg_drop_b):
        ...
        if len(res.keys()) > 0:
            ...
            if "graph_df" in res:
                ...
                if (not self.expected_passfail_val and self.csv_name is not None):
                    res_list = []
                    test_input_list = []
                    pass_fail_list = []
                    interop_tab_data = self.json_get('/adb/')["devices"]
                    ...
                    for device in res_list:
                        ...
                        for row in rows:
                            if row['DeviceList'] == device and row[self.csv_direction + ' Mbps'].strip() != '':
                                ...
                                break
                        if not found:
                            logging.info(f"Pass fail for Device {device} not found in CSV. Using default value 0.3")
...
''')
    ...
    if (len(selected_groups) != len(selected_profiles)):
        ...
        logger.warning("Please enter the correct set of arguments")
        exit(0)
        ...
        logger.warning("Please provide ssid password and security for configuring devices")
        exit(0)
