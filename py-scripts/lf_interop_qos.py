...
EXAMPLES:   # Command Line Interface to run download scenario with tos : Voice
            ...
            ./lf_interop_qos.py --ap_name Cisco --mgr 192.168.207.78 --mgr_port 8080 --num_stations_5g 32 --radio_5g wiphy1
            ...
            ./lf_interop_qos.py --client_type Both --ap_name Cisco --mgr 192.168.207.78 --mgr_port 8080
                --num_stations_5g 4 --radio_5g wiphy1 --ssid_5g "NETGEAR_5G_wpa2" --passwd_5g Password@123 --security_5g wpa2 --bands 5g --upstream eth1 --test_duration 1m
                --download 100000000 --upload 100000000 --traffic_type lf_tcp --tos "BK,BE,VI,VO" --create_sta --timebreak 5s
            ...
            ./lf_interop_qos.py --client_type Both --ap_name Cisco --mgr 192.168.207.78 --mgr_port 8080
                --num_stations_2g 4 --radio_2g wiphy0 --ssid_2g "NETGEAR_2G_wpa2" --passwd_2g Password@123 --security_2g wpa2 --bands 2.4g --upstream eth1 --test_duration 1m
                --download 100000000 --upload 100000000 --traffic_type lf_tcp --tos "BK,BE,VI,VO" --create_sta --timebreak 5s

            # Run Download Scenario for "existing" virtual stations with all TOS values.
            ./lf_interop_qos.py --client_type Virtual --ap_name NETGEAR --mgr 192.168.207.78 --mgr_port 8080 --bands 5g  --upstream eth1 --test_duration 1m  --download 2000000 --traffic_type lf_tcp
                --tos "VO,BK,BE,VI" --use_existing_station_list --existing_station_list 1.1.sta9090,1.1.sta8888

            # Run Download Scenario for both real and "existing" virtual stations with all TOS values.
            ./lf_interop_qos.py --client_type Both --ap_name NETGEAR --mgr 192.168.207.78 --mgr_port 8080 --bands 5g  --upstream eth1 --test_duration 1m  --download 2000000 --traffic_type lf_tcp
                --tos "VO,BK,BE,VI" --use_existing_station_list --existing_station_list 1.1.sta9090,1.1.sta8888
...
class ThroughputQOS(Realm):
    def __init__(self,
        ...
        self.client_type = client_type       # We will be using these for report generation.
        self.macid_list = []
        self.mode_list = []
        self.bssid_list = []
        self.channels_list = []
    ...
    def build(self, client_type="Real"):
        ...
        logger.info(f"CX List after the build Method : {list(self.cx_profile.created_cx.keys())}")
    ...
    def build_virtual_stations(self):
        ...
        def resolve(band_ssid, band_passwd, band_sec):
            """
                This method takes the specified band specific ssid, password, security config information for virtual stations
                and validates them inorder to build 'Virtual Stations'.

                Sometimes the user wants to have same config for both real and virtual clients - then this method also supports the backward compatibility
                for configuring the virtual stations with real devices argument's specified.
            """
        ...
        def station_count(specific):
            """This method validates the 'Count' of virtual stations specified by the end-user."""
        ...
        for key in bands:
            ...
            if key in ("2.4G", "2.4g"):
                ...
                logger.info(f"Printing band specific ssid,passwd,sec from build() : {ssid} - {passwd} - {sec}")
    ...
    def create_cx_real(self):
        ...
        logger.info(f"Real_Client_List : {self.real_client_list}")
    ...
    def create_cx_virtual(self):
        ...
        logger.info(f"Direction (virtual): {self.direction}")
        traffic_type = (self.traffic_type.strip("lf_")).upper()
    ...
    def monitor(self, curr_coordinate=None, curr_rotation=None, monitor_charge_time=None,
        ...
        """
            This monitor method is an integrated method that works for --client type scenarios Real / Virtual / Both.

            This monitor method has support for
        """
        ...
        # This self.real_time_data dict is one that contains our live monitoring data obtained from the endpoint's.
        # So we are storing the data w.r.t to timestamp for overall throughput csv's generation for all the devices or virtual stations under the test.
        self.real_time_data = {
            cx: {
        ...
        def map_tos(tos_val):
            """
                This method is added inorder to resolve the 'key-errors' that are being generated from the qos script.
                While monitoring the endpoint data in 'postman' - observed that sometimes we are getting 'tos' key as an integer or as a string.
                Mapped the 'TOS' keys - values dict - by the help of L3 cx create tab in the Lanforge Manager.
                So we have added this validating - for passing the correct tos value.
            """
            _map = {
                "100": "BE", 100: "BE",
                "32": "BK", 32: "BK",
        ...
        def resolve_client_key(cx_name):
            """
                This method is used to get the correct real devices and virtual clients under the test and use them for reporting.
            """
            if self.sta_list:
                for sta in self.sta_list:
                    if sta in cx_name:
        ...
        while datetime.now() < end_time or getattr(self, "background_run", None):
            ...
            # This API Call to calculate CLient Avg RTT. (New addon column for Overall CSV'S reporting.)
    ...
    def set_report_data_virtual(self, data):
        """
            This method is used to set report_data inorder to generate reports for virtual stations only.
        """
        rate_down = str(str(int(self.cx_profile.side_b_min_bps) / 1000000) + ' ' + 'Mbps')
        rate_up = str(str(int(self.cx_profile.side_a_min_bps) / 1000000) + ' ' + 'Mbps')
        if data is None:
    ...
    def generate_report(self, data, input_setup_info,
        ...
        logger.info("Printing Port Manager Tab Data : ")
        logger.info(f"{self.ssid_list}  --- {self.macid_list} --- {self.mode_list} --- {self.bssid_list} --- {self.channels_list} --- {self.rssi_list}")
        ...
        logger.info("Printing Throughput Table DataFrame : ")
        print(res["throughput_table_df"])
        ...
        logger.info(f"Overall Throughput Graph : {(graph_png)}")
    ...
    def parse_timebreak(self, tb_str):
        """This method is used to validate the timebreak passed by the end-user."""
        if not tb_str:
            return None
...
def validate_args(args):
    ...
    logger.info(f"Client_type : {args.client_type}")
    if args.group_name:
...
''')
    ...
    if args.download and args.upload:
        ...
        if args.upload:
            ...
            logger.info(f"Loads Data Upload : {loads_data}")
    if args.download and args.upload:
    ...
    for index in range(len(loads_data)):
        ...
        if args.client_type == "Both":
            ...
            throughput_qos.ssid_list, throughput_qos.macid_list, throughput_qos.mode_list = ssid_list, mac_list, mode_list
            throughput_qos.bssid_list, throughput_qos.channels_list, throughput_qos.rssi_list = bssid_list, channel_list, rssi_list
            logger.info(f"Total Test Devices : {total_devices_list}")
        elif args.client_type == 'Virtual':
            logger.info(f"Station List : {throughput_qos.sta_list}")
            ssid_list, mac_list, mode_list, bssid_list, channel_list, rssi_list = throughput_qos.get_portmgr_data(throughput_qos.sta_list)
            throughput_qos.ssid_list, throughput_qos.macid_list, throughput_qos.mode_list = ssid_list, mac_list, mode_list
            throughput_qos.bssid_list, throughput_qos.channels_list, throughput_qos.rssi_list = bssid_list, channel_list, rssi_list
        else:
            logger.info(f"Input Real Device List : {throughput_qos.input_devices_list}")
