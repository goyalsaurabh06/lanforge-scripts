...
class VideoStreamingTest(Realm):
        ...
        try:
            ...
            if data:
                if len(self.created_cx.keys()) > 1:
                    ...
                    if endpoint:
                        cx_metrics[list(self.created_cx.keys())[0]] = endpoint
    ...
    def generate_report(self, date, iterations_before_test_stopped_by_user, test_setup_info, realtime_dataset, report_path='', iot_summary=None):
        ...
        if self.device_issue_log:
            report.set_obj_html("Device Issues During Test",
                                "The table below lists devices for which data was temporarily unavailable "
                                "during the test (e.g. a disconnect), along with the time each issue was "
                                "first observed. The test continued for the remaining devices.")
            report.build_objective()
            issues_df = pd.DataFrame(self.device_issue_log)
            report.set_table_dataframe(issues_df)
            report.build_table()
        report.build_footer()
        report.write_html()
        report.write_pdf()
