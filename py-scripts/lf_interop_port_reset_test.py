...
class InteropPortReset(Realm):
    def __init__(
        ...
        self.lf_report = lf_report_pdf.lf_report(
            _path="" if not self.dowebgui else self.result_dir,
            _results_dir_name="Interop_port_reset_test",
            _output_html="port_reset_test.html",
            _output_pdf="port_reset_test.pdf",
        )
        self.report_path = self.lf_report.get_report_path()

        self.base_interop_profile = base.RealDevice(
            manager_ip=self.lanforge_ip,
            server_ip=self.upstream_port,
            ssid_5g=self.ssid,
            encryption_5g=self.encryption,
            passwd_5g=self.passwd,
            disconnect_devices=self.forget_network,
            reboot=False,
            selected_bands=["5g"],
        )
    ...
    def json_get_with_retry(self, url, wait_time=40, poll_interval=5, debug_=False):
        ...
        while response is None and (time.time() - start_time) < wait_time:
            logging.warning(
                f"GET {url} returned no response from LANforge; retrying..."
            )
    ...
    def change_port_to_ip(self, upstream_port):
        ...
        if upstream_port.count(".") != 3:
            ...
            response = self.json_get_with_retry(
                f"/port/{shelf}/{resource}/{port_name}?fields=ip"
            )
            try:
                ...
                logging.critical(
                    "Aborting the test: a valid manager IP is required to proceed."
                )
                exit(1)
    ...
    def selecting_devices_from_available(self):
        ...
        asyncio.run(
            self.base_interop_profile.configure_wifi(
                configurable_devices[0]
                + configurable_devices[1]
                + configurable_devices[2]
            )
        )
        ...
        if len(self.real_station_list) == 0:
            logging.error(
                "There are no real devices in this testbed. Aborting the test."
            )
        ...
        self.interop = base.BaseInteropWifi(
            manager_ip=self.lanforge_ip,
        ...
        for device_name in supported_resource_ids.keys():
            if device_name != "":
                if any(device_name in item for item in android_list):
                    self.final_selected_android_list.append(
                        supported_resource_ids[device_name]
                    )
        logging.info(
            f"Final Android Serial Numbers List: {self.final_selected_android_list}"
        )
                ...
                if port_name is None:
                    ...
                    if message_resource == resource_id:
                        if port_name in message_tokens:
                            ...
                            if f"IFNAME={port_name}" in message_tokens:
                                if match_text in message_tokens:
                                    matches.append("YES")
                                else:
                                    match_tokens = self.tokenize_wifi_msg(match_text)
    ...
    def collect_device_metrics(
        ...
        try:
            ...
            logging.warning(
                f"Defaulting device {device_eid} stats to 0 for this reset and continuing."
            )
            for metric_name in (
                "ConnectAttempt",
                "Disconnected",
            ...
            device_metrics[str(device_eid)][
                "Remarks"
            ] = "Data unavailable - LANforge API error, stats defaulted to 0"
        ...
        if type(wifi_messages) is not list:
            logging.info(
                f"Device {device_eid} : Getting wifi messages for only single time-stamp. Converting into List."
            )
            wifi_messages = [
                {
        ...
        if "1.1." in device_eid or is_android:
            # disconnects
            android_disconnect_count = self.count_wifi_msg_matches(
                wifi_messages=wifi_messages,
                message_keys=message_keys,
                device_eid=device_eid,
                match_text="Terminating...",
            )
            if android_disconnect_count > 1 or android_disconnect_count == 0:
                ...
                if device_state == "COMPLETED":
                    ...
                    logging.info(
                        "The Device %s is not in connected state." % device_eid
                    )
                    android_disconnect_count = 1
                logging.info(
                    f"Disconnect Count For Android: {android_disconnect_count}"
                )
            logging.info(
                "Final Disconnect count for %s: %s"
                % (device_eid, android_disconnect_count)
            )
            device_metrics[device_eid]["Disconnected"] = android_disconnect_count
            # scanning count
            android_scan_count = self.count_wifi_msg_matches(
                wifi_messages=wifi_messages,
                message_keys=message_keys,
                device_eid=device_eid,
            ...
            logging.info(
                "Final Scanning Count for %s: %s" % (device_eid, android_scan_count)
            )
            device_metrics[str(device_eid)]["Scanning"] = android_scan_count
            # association attempts
            android_association_attempt = self.count_wifi_msg_matches(
                wifi_messages=wifi_messages,
                message_keys=message_keys,
                device_eid=device_eid,
                match_text="ASSOCIATING",
            )
            logging.info(
                "Final Association Attempts Count for %s: %s"
                % (device_eid, android_association_attempt)
            )
            device_metrics[str(device_eid)][
                "ConnectAttempt"
            ] = android_association_attempt
            # association rejections
            android_association_rejection = self.count_wifi_msg_matches(
                wifi_messages=wifi_messages,
                message_keys=message_keys,
                device_eid=device_eid,
                match_text="ASSOC_REJECT",
            )
            logging.info(
                "Final Association Rejection Count for %s: %s"
                % (device_eid, android_association_rejection)
            )
            device_metrics[str(device_eid)][
                "Association Rejection"
            ] = android_association_rejection
            # connections
            android_connected_count = self.count_wifi_msg_matches(
                wifi_messages=wifi_messages,
                message_keys=message_keys,
                device_eid=device_eid,
                match_text="CTRL-EVENT-CONNECTED",
            )
            ...
            if android_association_attempt > android_connected_count:
                android_association_rejection = (
                    android_association_attempt - android_connected_count
                )
            device_metrics[str(device_eid)][
                "Association Rejection"
            ] = android_association_rejection
            if android_connected_count > 0:
                ...
                resource_id_response = self.json_get_with_retry(
                    "/adb/1/{}/{}?fields=resource-id".format(resource, serial)
                )
                port_response = None
                try:
                    resource_id = resource_id_response["devices"]["resource-id"]
                    port_response = self.json_get_with_retry(
                        "port/1/{}/wlan0?fields=cx time (us)".format(
                            resource_id.split(".")[1]
                        )
                    )
            ...
            if device_eid in self.windows_list:  # for windows
                win_disconnect_count = self.count_wifi_msg_matches(
                    wifi_messages=wifi_messages,
                    message_keys=message_keys,
                    device_eid=device_eid,
                    match_text="Wireless security stopped.",
                )
                ...
                logging.info(
                    "Final Disconnect count for %s: %s"
                    % (device_eid, win_disconnect_count)
                )
                device_metrics[device_eid]["Disconnected"] = win_disconnect_count
                win_scan_count = self.count_wifi_msg_matches(
                    wifi_messages=wifi_messages,
                    message_keys=message_keys,
                    device_eid=device_eid,
                    match_text="service started",
                )
                logging.info(
                    "Final Scanning Count for %s: %s" % (device_eid, win_scan_count)
                )
                device_metrics[str(device_eid)]["Scanning"] = win_scan_count
                win_association_attempt = self.count_wifi_msg_matches(
                    wifi_messages=wifi_messages,
                    message_keys=message_keys,
                    device_eid=device_eid,
                    match_text="association started.",
                )
                logging.info(
                    "Final Association Attempts Count for %s: %s"
                    % (device_eid, win_association_attempt)
                )
                device_metrics[str(device_eid)][
                    "ConnectAttempt"
                ] = win_association_attempt
                win_association_rejection = self.count_wifi_msg_matches(
                    wifi_messages=wifi_messages,
                    message_keys=message_keys,
                    device_eid=device_eid,
                    match_text="failed to connect",
                )
                logging.info(
                    "Final Association Rejection Count for %s: %s"
                    % (device_eid, win_association_rejection)
                )
                device_metrics[str(device_eid)][
                    "Association Rejection"
                ] = win_association_rejection
                win_connected_count = self.count_wifi_msg_matches(
                    wifi_messages=wifi_messages,
                    message_keys=message_keys,
                    device_eid=device_eid,
                    match_text="connected",
                )
                ...
                logging.info(
                    "Final Connected Count for %s: %s"
                    % (device_eid, win_connected_count)
                )
                device_metrics[str(device_eid)]["Connected"] = win_connected_count
                if win_association_attempt > win_connected_count:
                    win_association_rejection = (
                        win_association_attempt - win_connected_count
                    )
                device_metrics[str(device_eid)][
                    "Association Rejection"
                ] = win_association_rejection
                ...
                linux_association_attempt = self.count_wifi_msg_matches(
                    ...
                    message_keys=message_keys,
                    device_eid=device_eid,
                    match_text="<3>Trying to associate with",
                )
                logging.info(
                    "Final Association Attempts Count for %s: %s"
            ...
            for device_eid, metrics in iteration_metrics.items():
                ...
                for metric_name, value in metrics.items():
                    if metric_name in ("Remarks", "cx time (us)"):
                        ...
                        aggregated_metrics[device_eid][metric_name] += (
    ...
    def write_coordinate_csv(self, per_iteration_results, iteration):
        ...
        aggregated_metrics = self.aggregate_device_metrics(
        ...
        for device_eid, metrics in aggregated_metrics.items():
            ...
            self.coordinate_df[self.current_coordinate][client_name][
                "coordinate"
        ...
        df.to_csv(
            f"{self.report_path}/overall_reset_{self.current_coordinate}.csv",
            index=False,
        )
        if self.dowebgui:
            df.to_csv(
                f"{self.result_dir}/overall_reset_{self.current_coordinate}.csv",
                index=False,
            )
    ...
    def performing_resets(self, test_start_time=None):
        ...
        for iteration, _ in zip(range(self.iterations), per_iteration_results):
            if self.robot_test:
                ...
                if current_time >= next_battery_check:
                    ...
                    if paused:
                        ...
                        reached, abort = self.robot_obj.move_to_coordinate(
                            self.current_coordinate
                        )
            ...
            logging.info(
                f"Waiting until given {self.reset_interval_sec} sec time interval to finish..."
            )
            time.sleep(
                int(self.reset_interval_sec)
            )  # sleeping until time interval finish
            ...
            for device_eid in self.adb_device_list:
                device_metrics[device_eid] = empty_metrics.copy()
            for device_eid in self.all_laptops:
                laptop_metrics[device_eid] = empty_metrics.copy()
            ...
            for device_eid in self.all_selected_devices:
                iteration_metrics = self.collect_device_metrics(
                ...
                if self.robot_test:
                    self.write_coordinate_csv(
                        per_iteration_results=per_iteration_results, iteration=iteration
                    )
                else:
                ...
                if self.dowebgui:
                    with open(
                        self.result_dir
                        + f"/../../Running_instances/{self.lanforge_ip}_{self.test_name}_running.json",
                        "r",
                    ) as file:
            ...
            if test_stopped:
                ...
                for iteration_index in range(self.iterations):
                    if per_iteration_results.get(iteration_index) is None:
                        ...
                        for device_eid, metrics in per_iteration_results[
                            iteration_index
                        ].items():
                            if any(value is None for value in metrics.values()):
                                per_iteration_results[iteration_index][
                                    device_eid
                                ] = stopped_metrics.copy()
                ...
                break
        logging.info(
            f"Final Reset Count Dictionary for all clients: {per_iteration_results}"
        )
        ...
        if self.robot_test:
            if self.rotation_enabled:
                logging.info(
                    f"At coordinate {self.current_coordinate} on Angle {self.current_angle} Test Ended at {test_end}"
                )
            else:
                logging.info(
                    f"At coordinate {self.current_coordinate} Test Ended at {test_end}"
                )
        else:
            logging.info(f"Test Ended at {test_end}")
        time_format = "%b %d %H:%M:%S"
        test_duration = datetime.strptime(
            test_end_time, time_format
        ) - datetime.strptime(test_start_time, time_format)
        if self.robot_test:
            if self.rotation_enabled:
                logging.info(
                    f"Total Test Duration taken to complete port resets at coordinate {self.current_coordinate} on angle {self.current_angle}: {test_duration}"
                )
            else:
                logging.info(
                    f"Total Test Duration taken to complete port resets at coordinate {self.current_coordinate} : {test_duration}"
                )
        else:
    ...
    def run(self):
        ...
        try:
            ...
            logging.info(
                f"Final Active Devices List (Android, Windows, Linux, Mac) Which support user specified release & not in phantom : {self.adb_device_list, self.base_interop_profile.windows_list, self.base_interop_profile.linux_list, self.base_interop_profile.mac_list}"  # noqa: E501
            )
            self.all_selected_devices = (
                self.adb_device_list
                + self.windows_list
            ...
            if (
                len(self.adb_device_list) == 0
                and len(self.base_interop_profile.windows_list) == 0
                and len(self.base_interop_profile.linux_list) == 0
                and len(self.base_interop_profile.mac_list) == 0
            ):
                ...
                for index in range(len(self.adb_device_list)):
                    self.android_serials.append(
                        self.adb_device_list[index].split(".")[2]
                    )
                logging.info(
                    f"Separated device names from the full name: {self.android_serials}"
                )

            phantom_states = []
            for device_eid in self.adb_device_list:
                phantom_states.append(
                    self.interop.get_device_details(device=device_eid, query="phantom")
                )
            if (
                self.adb_device_list
                or self.windows_list
                or self.linux_list
                or self.mac_list
            ):
                for device_eid in self.adb_device_list:
                    self.android_user_names.append(
                        self.interop.get_device_details(
                            device=device_eid, query="user-name"
                        )
                    )
                logging.info(
                    f"ADB user-names for selected devices: {self.android_user_names}"
                )
                logging.info("Checking heath data...")
                android_health = dict.fromkeys(self.adb_device_list)
                logging.info(
                    f"Initial Health Data For Android Clients: {android_health}"
                )
                laptop_health = dict.fromkeys(self.all_laptops)
                logging.info(
                    f"Initial Health Data For Laptops Clients: {laptop_health}"
                )
                ...
                for device_eid in self.adb_device_list:
                    ...
                    if device_state == "COMPLETED,":
                        ...
                        if device_ssid == self.ssid:
                            logging.info(
                                "The Device %s is connected to expected ssid (%s)"
                                % (device_eid, device_ssid)
                            )
                            android_health[device_eid] = (
                                self.utility.get_wifi_health_monitor(
                                    device=device_eid, ssid=self.ssid
                                )
                            )
                        else:
                            logging.info(
                        ...
                        device_state = self.utility.get_device_state(device=device_eid)
                        logging.info(
                            "Checking Device Status Again..." + str(device_state)
                        )
                        logging.info(f"Device state {device_state}")
                        if device_state == "COMPLETED,":
                            logging.info("Phone is in connected state")
                            device_ssid = self.utility.get_device_ssid(
                                device=device_eid
                            )
                            if device_ssid == self.ssid:
                                logging.info(
                ...
                nav_data = os.path.join(
                    base_dir, "nav_data.json"
                )  # To generate nav_data.json in webgui folder
                ...
                for coordinate_index in range(len(self.coordinate_list)):
                    ...
                    if test_stopped_by_user:
                        break
                    reached_coordinate, abort = self.robot_obj.move_to_coordinate(
                        self.coordinate_list[coordinate_index]
                    )
                    ...
                    if reached_coordinate:
                        ...
                        if not self.rotation_enabled:
                            per_iteration_results, test_duration = (
                                self.performing_resets(test_start_time=test_start_time)
                            )
                            self.port_reset_data[
                                self.coordinate_list[coordinate_index]
                            ] = {
                            ...
                            for angle_index in range(len(self.rotation_list)):
                                ...
                                _, test_stopped_by_user = (
                                    self.robot_obj.wait_for_battery()
                                )
                                rotated = self.robot_obj.rotate_angle(
                                    self.rotation_list[angle_index]
                                )
                                if rotated:
                                    self.current_angle = self.rotation_list[angle_index]
                                    per_iteration_results, test_duration = (
                                        self.performing_resets(
                                            test_start_time=test_start_time
                                        )
                                    )
                                if (
                                    self.coordinate_list[coordinate_index]
                                    not in self.port_reset_data
                                ):
                                    self.port_reset_data[
                                        self.coordinate_list[coordinate_index]
                                    ] = {}
    ...
    def generate_overall_graph(
        ...
        for device_eid in self.adb_device_list:
            ...
            for iteration in per_iteration_results:
                if device_eid in list(per_iteration_results[iteration].keys()):
                    connected.append(
                        per_iteration_results[iteration][device_eid]["Connected"]
                    )
                    disconnected.append(
                        per_iteration_results[iteration][device_eid]["Disconnected"]
                    )
                    scanning.append(
                        per_iteration_results[iteration][device_eid]["Scanning"]
                    )
                    assoc_attempts.append(
                        per_iteration_results[iteration][device_eid]["ConnectAttempt"]
                    )
                    assoc_rejections.append(
                        per_iteration_results[iteration][device_eid][
                            "Association Rejection"
                        ]
                    )
        ...
        for index, _ in zip(
            range(len(connected_counts)), range(len(disconnected_counts))
        ):
        ...
        for device_eid, device_index in zip(device_list, range(len(device_list))):
            ...
            if "1.1." in device_eid:
                ...
                self.lf_report.set_obj_html(
                    "Port Resets for Client "
                    + str(android_user_name)
                    + " ("
                    + str(device_eid.split(".")[2])
                    + ")",
                    "The below table & graph displays details of "
                    + str(android_user_name)
                    + " device.",
                )
            ...
            client_graph = self.per_client_graph(
                metric_totals=metric_totals,
                image_name="per_client_" + str(device_index),
                figsize=(13, 5),
                _alignment={"left": 0.1},
                remove_border=["top", "right"],
    ...
    def generate_report(self, per_iteration_results=None, test_duration=None):
        ...
        try:
            ...
            self.lf_report.test_setup_table(
                value="Basic Test Information", test_setup_data=test_setup_info
            )

            self.lf_report.set_obj_html(
                "Overall Port Resets Graph",
            ...
            self.generate_overall_graph_table(
            ...
            for device_eid in self.all_laptops:
                ...
                user_names.append(
                    self.interop.get_laptop_devices_details(
                        device=device_eid, query="host_name"
                    )
                )
                hw_version = self.interop.get_laptop_devices_details(
                    device=device_eid, query="hw_version"
                )
            ...
            if self.dowebgui:
                ...
                self.lf_report.write_pdf_with_timestamp(
                    _page_size="A4", _orientation="Portrait"
                )
    ...
    def add_live_view_images_to_report(self):
        ...
        for floor in range(0, int(self.total_floors)):
            port_reset_image_path = os.path.join(
                self.result_dir,
                "live_view_images",
                f"port_reset_{self.test_name}_{floor + 1}.png",
            )
            timeout_sec = 60
            start_time = time.time()
            ...
            if os.path.exists(port_reset_image_path):
                ...
                self.lf_report.set_custom_html(
                    f'<img src="file://{port_reset_image_path}"></img>'
                )
                self.lf_report.build_custom()
        ...
        for coordinate_index in range(len(self.coordinate_list)):
            if self.rotation_enabled:
                for angle_index in range(len(self.rotation_list)):
                    ...
                    if (
                        coordinate not in self.port_reset_data
                        or angle not in self.port_reset_data[coordinate]
                        or "per_iteration_results"
                        not in self.port_reset_data[coordinate][angle]
                    ):
                        continue
                    per_iteration_results = self.port_reset_data[coordinate][angle][
                        "per_iteration_results"
                    ]
                    self.lf_report.set_obj_html(
                        _obj_title=f"Overall Port reset stats at Coordinate: {self.coordinate_list[coordinate_index]} | Rotation Angle: {self.rotation_list[angle_index]}°",
                        _obj="",
                    )
                    self.lf_report.build_objective()
                    ...
                    device_names, device_types, model, user_names, release = (
                        [],
                        [],
                        [],
                        [],
                ...
                if (
                    coordinate not in self.port_reset_data
                    or "per_iteration_results" not in self.port_reset_data[coordinate]
                ):
                    continue
                per_iteration_results = self.port_reset_data[coordinate][
                    "per_iteration_results"
                ]
                self.lf_report.set_obj_html(
                    _obj_title=f"Overall Port reset stats at Coordinate: {self.coordinate_list[coordinate_index]}",
                    _obj="",
                )
                self.lf_report.build_objective()
    ...
    def write_overall_csv(self, per_iteration_results):
        ...
        for _, iteration_metrics in per_iteration_results.items():
            ...
            for device_eid, metrics in iteration_metrics.items():
                ...
                totals_per_device[device_eid]["ConnectAttempt"] += (
                    metrics.get("ConnectAttempt", 0) or 0
                )
                totals_per_device[device_eid]["Disconnected"] += (
                    metrics.get("Disconnected", 0) or 0
                )
                totals_per_device[device_eid]["Scanning"] += (
                    metrics.get("Scanning", 0) or 0
                )
                totals_per_device[device_eid]["Association Rejection"] += (
                    metrics.get("Association Rejection", 0) or 0
                )
                totals_per_device[device_eid]["Connected"] += (
                    metrics.get("Connected", 0) or 0
                )
                totals_per_device[device_eid]["Iterations"] += 1

        summary_df = pd.DataFrame.from_dict(
            totals_per_device, orient="index"
        ).reset_index()
...
""",
    ...
    parser.add_argument(
        "--lanforge_port", default="8080", help="Specify the LANforge manager HTTP port"
    )
    ...
    parser.add_argument(
        "--device_list",
        help="Enter the devices on which the test should be run",
        default=None,
    )
    ...
    parser.add_argument(
        ...
        "So, mention this argument when enterprise securities are selected.",
        default=None,
        action="store_true",
    )
    ...
    port_reset_test = InteropPortReset(
    ...
    if args.dowebgui:
        ...
        if port_reset_test.robot_test:
            port_reset_test.result_df.to_csv(
                f"{port_reset_test.report_path}/overall_reset_{port_reset_test.current_coordinate}.csv",
                index=False,
            )
            port_reset_test.result_df.to_csv(
                f"{port_reset_test.result_dir}/overall_reset_{port_reset_test.current_coordinate}.csv",
                index=False,
            )
        else:
            port_reset_test.result_df.to_csv(
                f"{port_reset_test.report_path}/overall_reset.csv", index=False
            )
            port_reset_test.result_df.to_csv(
                f"{port_reset_test.result_dir}/overall_reset.csv", index=False
            )
    if port_reset_test.robot_test:
